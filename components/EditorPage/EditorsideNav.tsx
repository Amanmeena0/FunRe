...
export default function EditorSideNav({ activeTab, onTabChange }: SideNavProps) {
  return (
    <aside className="hidden lg:flex flex-col h-[calc(100vh-64px-56px)] fixed left-0 top-[64px] w-56 bg-[#f9f9f9] dark:bg-[#1b1b1b] border-r-4 border-black z-40 print:hidden overflow-y-auto justify-between">
      <nav className="grow">
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-4 px-4 py-3 font-medium border-b-2 transition-colors cursor-pointer ${
                isActive
